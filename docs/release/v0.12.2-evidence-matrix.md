...
Current-note, added April 29, 2026: repo `main` has since moved the flake and
CI workflows to `nix-cache.tinyland.dev`. This section remains the historical
`v0.12.2` evidence record, not current cache configuration truth.
...
**Root cause:** the then-configured `nix-cache.fuzzy-dev.tinyland.dev` endpoint
was a tinyland-hosted Attic instance that did not resolve externally. Any Nix
install off a non-tinyland host fell back to full local build.
