...
```text
seaweedfs://100.64.48.53:8333/tcfs/neo-honey-conflict-sibling-20260510T051328Z
```
...
pattern, and optional sibling mode proves per-path sibling progress while another
path is conflicted. These do not prove daemon-backed `tcfs resolve` UX, Finder
conflict UX, automatic resolution, keep-synced/pin policy, or production
FileProvider status badges.
